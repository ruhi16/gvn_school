...
<aside class="w-56 shrink-0 border-r border-slate-200 bg-slate-950 text-slate-300"
    x-data="{ examOpen: {{ $examSettingsActive ? 'true' : 'false' }} }">
    ...
    <nav class="space-y-1 p-3 text-xs">
        ...
        <a href="{{ route('admin.shreny-sections') }}" class="block rounded px-3 py-2 hover:bg-slate-900">Shreny
            assignments</a>
        <a href="{{ route('admin.shreny-subjects') }}" class="block rounded px-3 py-2 hover:bg-slate-900">Shreny
            subjects</a>
        <a href="{{ route('admin.students') }}"
            class="block rounded px-3 py-2 {{ request()->routeIs('admin.students*') ? 'bg-cyan-950 text-cyan-300 font-semibold' : 'hover:bg-slate-900' }}">StudentDB</a>
