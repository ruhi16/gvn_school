<div class="space-y-5">
    ...
    @else
        <div class="space-y-6">
            @forelse ($shrenySections as $shrenySection)
                ...
                @if ($shreny && $section)
                    <section wire:key="script-distribution-{{ $shrenySection->id }}" class="overflow-hidden rounded-lg border border-slate-200 bg-white">
                            ...
                            <table class="min-w-[70rem] text-left text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse ($mappedSubjects as $mappedSubject)
                                        ...
                                        @if ($subject)
                                            <tr wire:key="script-row-{{ $shrenySection->id }}-{{ $subject->id }}" class="align-top hover:bg-slate-50/60">
                                                <td class="sticky left-0 bg-white px-4 py-4 font-medium text-slate-900">{{ $subject->name }}</td>
                                                @foreach ($configurations as $configuration)
                                                    @php($key = $shrenySection->shreny_id . ':' . $shrenySection->section_id . ':' . $subject->id . ':' . $configuration->exam_name_id . ':' . $configuration->exam_type_id . ':' . $configuration->exam_part_id)
                                                    @php($distribution = $distributions[$key] ?? null)
                                                    <td class="px-4 py-3">
                                                        <button type="button" wire:click="openTeacherModal({{ $shrenySection->shreny_id }}, {{ $shrenySection->section_id }}, {{ $subject->id }}, {{ $configuration->exam_name_id }}, {{ $configuration->exam_type_id }}, {{ $configuration->exam_part_id }})" class="w-full rounded border border-slate-300 px-3 py-2 text-left text-xs hover:border-cyan-500 hover:bg-cyan-50">
                                                            @if ($distribution && isset($teachers[$distribution->teacher_id]))
                                                                <span class="font-medium text-cyan-700">{{ $teachers[$distribution->teacher_id]->name }}</span>
                                                            @else
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" wire:click.self="closeModal">
            <section class="w-full max-w-md rounded-lg bg-white p-5 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="teacher-modal-title">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <button type="button" wire:click="closeModal" class="text-xl leading-none text-slate-400 hover:text-slate-700" aria-label="Close">&times;</button>
                </div>
                <div class="mt-5">
                    <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">Teacher</label>
                    <select wire:model="selectedTeacherId" class="mt-2 w-full rounded border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-cyan-500 focus:ring-cyan-500">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    <button type="button" wire:click="closeModal" class="rounded border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">Cancel</button>
                    <button type="button" wire:click="saveTeacherAssignment" class="rounded bg-cyan-600 px-3 py-2 text-sm font-semibold text-white hover:bg-cyan-700">Save assignment</button>
                </div>
            </section>
        </div>
    @endif
