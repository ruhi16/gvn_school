    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table class="min-w-[78rem] text-left text-sm">
            <thead class="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
                <tr>
                    ...
                    <th class="min-w-64 px-3 py-3">
                        ...
                        @if ($configuration->exam_mode_id)<span class="block font-normal text-cyan-700">Mode
                            selected</span>@endif
                    </th>
                    @endforeach
                </tr>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                @php($serial++)
                <tr wire:key="marks-{{ $shreny->id }}-{{ $shrenySubject->subject_id }}"
                    ...
                    <td class="sticky left-40 bg-white px-3 py-4 text-slate-700">{{
                        $subjects[$shrenySubject->subject_id]->name }}</td>
                    @foreach ($configurations as $configuration)
                    @php($key = $shreny->id . ':' . $shrenySubject->subject_id . ':' . $configuration->exam_name_id .
                ...
                <tr>
                    <td colspan="{{ 3 + $configurations->count() }}"
                        class="px-5 py-10 text-center text-sm text-slate-500">No shreny subjects available.</td>
                </tr>
                @endforelse
                @if ($serial === 0)
                <tr>
