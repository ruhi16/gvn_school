    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table class="min-w-full text-left text-sm">
            <thead class="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
                <tr>
                    <th class="w-16 px-4 py-3 font-semibold">SL</th>
                    <th class="min-w-48 px-4 py-3 font-semibold">Shreny</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr wire:key="shreny-subjects-{{ $shreny->id }}" class="align-top hover:bg-slate-50/60">
                    ...
                    <td class="px-4 py-4">
                        <p class="font-semibold text-slate-900">{{ $shreny->name }}</p>
                    ...
                    <td class="px-4 py-2">
                        <div class="grid gap-x-6 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <label
                                class="flex cursor-pointer items-center gap-3 border-b border-slate-100 px-1 py-3 text-sm transition hover:bg-slate-50">
                                <input type="checkbox"
                                    wire:click="toggleAssignment({{ $shreny->id }}, {{ $subject->id }})"
                                    @checked(in_array($subject->id, $selectedSubjects, true))
                                @disabled(!$mutationsEnabled) class="h-4 w-4 rounded border-slate-300 text-cyan-600
                                focus:ring-cyan-500">
                            ...
                            @endforelse
                            @if ($showAssignedOnly && count($selectedSubjects) === 0)<p
                                class="px-1 py-4 text-sm text-slate-500">No subjects assigned.</p>@endif
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="3" class="px-5 py-10 text-center text-sm text-slate-500">No shrenies available.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
