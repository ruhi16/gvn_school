<div class="space-y-5">
    ...
    <div class="space-y-6">
        ...
        @if ($shreny && $section)
        <section wire:key="marks-entry-{{ $shrenySection->id }}"
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
            <div>
                <button type="button" wire:click="closeEntry"
                    class="mb-2 text-xs font-semibold text-cyan-700 hover:text-cyan-900">&larr; Back to
                    combinations</button>
                <h3 class="font-semibold text-slate-900">{{ $selectedShreny?->name }} <span
                        class="font-normal text-slate-500">&amp;</span> {{ $selectedSection?->name }}</h3>
                <p class="text-xs text-slate-500">{{ $selectedSubject?->name }} · {{
                    $examNames[$selectedExamNameId]?->name }} / {{ $examTypes[$selectedExamTypeId]?->name }} / {{
                    $examParts[$selectedExamPartId]?->name }}</p>
            </div>
            <div class="flex items-center gap-2">
                ...
                @if ($isFinalized)
                <span
                    class="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">Finalized</span>
                @endif
                @if ($isIssued)
                <button type="button" wire:click="unfinalizeMarks"
                    class="rounded border border-amber-300 px-3 py-2 text-xs font-semibold text-amber-800 hover:bg-amber-50">Unfinalize</button>
                @elseif (!$isFinalized)
                <button type="button" wire:click="finalizeMarks"
                    class="rounded bg-cyan-600 px-3 py-2 text-xs font-semibold text-white hover:bg-cyan-700">Finalize
                    marks</button>
                @endif
            </div>
        </div>
            ...
            <table class="min-w-[85rem] text-left text-sm">
                <thead class="bg-white text-xs uppercase tracking-wider text-slate-500">
                    <tr>
                        ...
                        @php($combinationPrefix = $combination->exam_name_id . ':' . $combination->exam_type_id . ':' .
                        $combination->exam_part_id . ':')
                        @php($combinationTeacher = $selectedCombinationTeachers[$combination->exam_name_id . ':' .
                        $combination->exam_type_id . ':' . $combination->exam_part_id] ?? null)
                        @php($combinationState = $selectedCombinationStates[$combinationPrefix] ?? ['is_finalized' =>
                        false, 'is_issued' => false])
                        @php($isCurrentCombination = $combination->exam_name_id === $selectedExamNameId &&
                        $combination->exam_type_id === $selectedExamTypeId && $combination->exam_part_id ===
                        $selectedExamPartId)
                        <th
                            class="min-w-64 border-b border-slate-200 px-4 py-3 {{ $isCurrentCombination ? 'bg-cyan-50' : '' }}">
                            <span class="block text-slate-900">{{ $examNames[$combination->exam_name_id]?->name
                                }}</span>
                            <span class="font-normal">{{ $examTypes[$combination->exam_type_id]?->name }} / {{
                                $examParts[$combination->exam_part_id]?->name }}</span>
                            <span class="mt-1 block normal-case font-medium text-cyan-700">Teacher: {{
                                $combinationTeacher?->teacher_id ? ($teachers[$combinationTeacher->teacher_id]?->name ??
                                'Not assigned') : 'Not assigned' }}</span>
                            <span
                                class="mt-1 block normal-case font-medium {{ $combinationTeacher?->is_finalized ? 'text-emerald-700' : 'text-blue-700' }}">{{
                                $combinationTeacher?->is_finalized ? 'Finalized' : 'Not finalized' }}{{
                                $combinationTeacher?->is_issued ? ' · Issued' : '' }}</span>
                            @if ($combinationTeacher?->allotted_date)
                            @php($isOverdue = $combinationTeacher->allotted_date->isBefore(today()))
                            <span
                                class="mt-1 block normal-case text-xs {{ $isOverdue ? 'font-semibold text-red-600' : 'text-slate-500' }}">Due:
                                {{ $combinationTeacher->allotted_date->format('Y-m-d') }}</span>
                            @endif
                        </th>
                        @endforeach
                    </tr>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr wire:key="mark-student-{{ $student->id }}" class="hover:bg-slate-50/60">
                        ...
                        <td class="px-4 py-3 {{ $isCurrentCombination ? 'bg-cyan-50/40' : '' }}">
                            ...
                            <span
                                class="font-medium {{ $combinationEntry->obtained_marks === -99 ? 'text-rose-600' : 'text-slate-700' }}">{{
                                $combinationEntry->obtained_marks === -99 ? 'AB' : $combinationEntry->obtained_marks
                                }}</span>
                            @else
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ 2 + $selectedCombinations->count() }}"
                            class="px-4 py-8 text-center text-sm text-slate-500">No students are assigned to this Shreny
                            and Section.</td>
                    </tr>
                    @endforelse
                </tbody>
