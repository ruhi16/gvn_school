...
class ExamTypeComp extends Component
{
    ...
    public function save(): void { if (!$this->canMutate()) return; $editing = $this->recordId !== null; $data = $this->validate(['name' => ['required', 'string', 'max:255'], 'description' => ['nullable', 'string', 'max:255'], 'order_id' => ['nullable', 'integer'], 'school_id' => ['nullable', 'integer'], 'is_active' => ['boolean'], 'remarks' => ['nullable', 'string', 'max:255']]); ExamType::updateOrCreate(['id' => $this->recordId], $data); $this->showModal = false; $this->resetForm(); session()->flash('success', $editing ? 'Exam type updated.' : 'Exam type created.'); }
    public function delete(int $id): void { if (!$this->canMutate()) return; ExamType::findOrFail($id)->delete(); session()->flash('success', 'Exam type deleted.'); }
