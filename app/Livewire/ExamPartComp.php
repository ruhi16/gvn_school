...
class ExamPartComp extends Component
{
    ...
    public function save(): void { $editing = $this->recordId !== null; $data = $this->validate(['name' => ['required', 'string', 'max:255'], 'description' => ['nullable', 'string', 'max:255'], 'order_id' => ['nullable', 'integer'], 'school_id' => ['nullable', 'integer'], 'is_active' => ['boolean'], 'remarks' => ['nullable', 'string', 'max:255']]); ExamPart::updateOrCreate(['id' => $this->recordId], $data); $this->showModal = false; $this->resetForm(); session()->flash('success', $editing ? 'Exam part updated.' : 'Exam part created.'); }
    public function delete(int $id): void { ExamPart::findOrFail($id)->delete(); session()->flash('success', 'Exam part deleted.'); }
